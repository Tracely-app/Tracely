...
- `bridge-notes.md` — the spike's wiring plan. As shipped, the protocol adds the
  ...
  `undo` takes `undoToken` (one or an array, newest first; `rollback: true` only for a failed
  group's immediate take-back), hints travel as `hint: {…}`, a `find` matches only whole
  sentences, and a hint's `occurrences` that disagrees with the live doc is refused as `stale`.
...
- `hook-trial.mjs` — drives the SHIPPED `extension/docs-hook.js` in a real Doc over content.js's
  ...
  again after the review fixes; live Doc verified unchanged afterwards both times).
