...
try {
  ...
  // ── what content.js sees: the ping's verdict on THIS doc ──
  check("ping reports the doc as editable (api + editor, not view-only)", ping.api && ping.editor && ping.viewOnly === false, { mode: ping.mode, status: ping.status });

  // A REAL revision, as the model writes them: curly quotes, an apostrophe,
  // an em dash, a comma number, a parenthetical. The Zq trial strings never
  // carried any of these, and the read-back has to survive them.
  const sents = [];
  for (const para of T0.split(/[\n\u0003]/)) {
    const re = /[^.!?]+(?:[.!?]+["')\]]*|$)/g;
    let m;
    while ((m = re.exec(para))) { const s = m[0].trim(); if (s.split(/\s+/).length >= 6 && /[.!?]["')\]]*$/.test(s) && T0.split(s).length === 2) sents.push(s); }
  }
  const [A, B] = sents;
  check("two unique sentences to work on", A && B, { A, B });
  const REV = "Maverick\u2019s return \u2014 ordered by Admiral \u201cCyclone\u201d Simpson (Jon Hamm) \u2014 puts him in charge of 1,200 hours of training, and he isn\u2019t happy about it.";
  await page.evaluate(() => { window.__tracelyEditConfig.allowEdits = true; });
  const r1 = await send(page, "replace", { find: A, replacement: REV, hint: { occurrence: 0, occurrences: 1 } });
  const t1 = await text(page);
  check("a realistic revision (curly quotes, em dash, apostrophe, 1,200) lands and verifies exact", r1.ok && r1.verified === "exact" && t1.includes(REV), { ok: r1.ok, verified: r1.verified, reason: r1.reason, ms: r1.ms, landed: t1.includes(REV) });
  const u1 = await send(page, "undo", { undoToken: [r1.undoToken] });
  check("undo restores the exact baseline", u1.ok && (await text(page)) === T0, { reason: u1.reason });

  // The marker path content.js uses for a cited sentence: a [3] must survive the rewrite.
  const withMarker = B.replace(/([.!?]+["')\]]*)$/, " [3]$1");
  const r2 = await send(page, "replace", { find: B, replacement: withMarker, hint: { occurrence: 0, occurrences: 1 } });
  check("adding a [3] marker to a sentence lands", r2.ok, { reason: r2.reason, verified: r2.verified });
  const REV2 = "It\u2019s a strong story line \u2014 well acted, with some of the most \u201cimmersive\u201d flight footage ever shot [3].";
  const r3 = await send(page, "replace", { find: withMarker, replacement: REV2, hint: { occurrence: 0, occurrences: 1 } });
  check("rewriting the marked sentence keeps the marker and verifies", r3.ok && (await text(page)).includes(REV2), { reason: r3.reason, verified: r3.verified, ms: r3.ms });
  const u2 = await send(page, "undo", { undoToken: [r3.undoToken, r2.undoToken] });
  check("undo (both) restores the exact baseline", u2.ok && (await text(page)) === T0, { reason: u2.reason });
} catch (e) {
